! function (t) {
  ...
  function a(t) {
    if (!((new Date).getTime() < f + s)) {
      ...
      if (r(g[0].obj.get(0).parentNode)) {
        ...
        var h, b, y, w, x, R = document.body.getBoundingClientRect(),
          F = i / (e() === !0 ? m : R.width),
          S = p / (e() === !0 ? m : R.height);
  ...
  t.plaxify = function (t, r) {
    return function () {
      ...
      for (var o = 0; o < g.length; o++) this === g[o].obj.get(0) && (e = o);
  ...
  var d = function () {
    ...
    for (var e in r) void 0 !== n.style[e] && (n.style[e] = "translate3d(1px,1px,1px)", t = window.getComputedStyle(n).getPropertyValue(r[e]));
...
if (auth) {
  ...
  xhr.open('GET', '/sessions/login_404?return_to=' + encodeURIComponent(window.location.pathname), true)
  xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest')
  xhr.onload = function () {
    auth.innerHTML = xhr.responseText
    auth.querySelector('#login_field').setAttribute('placeholder', 'Username or Email')
    auth.querySelector('#password').setAttribute('placeholder', 'Password')
  ...
  document.addEventListener('keydown', function (event) {
    if (event.target === document.body && event.keyCode === BACKTICK && !event.metaKey) {
      var wrapper = document.getElementById('parallax_wrapper')
      wrapper.style.cssText += ';-webkit-filter:grayscale(50%)'
